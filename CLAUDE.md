...
- **Single parse, two consumers.** One debounced md4c parse per edit cycle feeds an `MdParseSink` fan-out. `scanner/` consumes spans and produces `MdStyleRun[]` for the source pane's coloring. `render/` consumes the full event stream and builds a `RenderModel` for the Read pane. Never parse twice — it's expensive at 25 MHz.
