...
Provide the classic Mac application shell for Armadillo Editor: Toolbox initialization, the main event loop, the menu bar, command dispatch, the About box, graceful quit, and the debounce poll that drives the parse pipeline. This capability is the wiring that makes a `.APPL` feel like a native System 7 application — nothing here is unique to a markdown editor; it's infrastructure that every well-behaved System 7 app needs.

The app shell owns the `MacSyscalls` real wiring (the function-pointer struct whose real implementations call the Toolbox), the editor window, and the main loop. It does not own document data, rendering logic, or text-engine state — those are delegated to the modules under `src_pane/`, `render/`, and `src/doc`, `src/file_io`, `src/win_editor`.
