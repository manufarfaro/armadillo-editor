...
Provide open and save functionality for `.md` files on HFS via the Mac Toolbox File Manager and Standard File Package. This capability wraps `SFGetFile` / `StandardGetFile`, `SFPutFile` / `StandardPutFile`, and raw File Manager I/O calls (`FSpOpenDF`, `FSRead`, `FSWrite`, `FSClose`) behind a vendor-free interface that `doc-store` and the app's command dispatch use.

`file-io` enforces the MVP document size limit (`kMaxDocBytes = 32767`) at open time, translating over-limit files into `kFileIoErrTooBig` so the UI can alert the user clearly instead of having the source pane silently truncate or crash.
